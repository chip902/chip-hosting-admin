...
const CalendarSyncComponent: React.FC<CalendarSyncProps> = ({ onEventsLoaded, onSyncComplete }) => {
	...
	const authenticateApple = async () => {
		try {
			...
			// Store current URL to return after auth
			sessionStorage.setItem("calendarReturnUrl", window.location.href);
			// Redirect to Apple auth
			window.location.href = authUrl;
		} catch (err: any) {
			setError("Apple authentication failed: " + err.message);
		}
	};

	// Handle Exchange/Mailcow authentication
	const authenticateExchange = async () => {
		try {
			const authUrl = await calendarClient.getAuthUrl(CalendarProvider.EXCHANGE);
			// Store current URL to return after auth
			sessionStorage.setItem("calendarReturnUrl", window.location.href);
			// Redirect to Exchange auth
			window.location.href = authUrl;
		} catch (err: any) {
			setError("Exchange authentication failed: " + err.message);
		}
	};
	...
	const handleAuthCallback = async (provider: CalendarProvider, code: string, state?: string) => {
		try {
			...
			if (provider === CalendarProvider.GOOGLE) {
				...
				setAppleAuth(true);
			} else if (provider === CalendarProvider.EXCHANGE) {
	...
	const updateSyncSource = async (sourceId: string, updates: Partial<SyncSource>) => {
		try {
			...
			setSyncSources((prev) => prev.map((source) => (source.id === sourceId ? result : source)));
		...
		try {
			const credentials =
				providerId === CalendarProvider.GOOGLE
					...
					? calendarClient["credentials"][CalendarProvider.MICROSOFT]
					: providerId === CalendarProvider.APPLE
					? calendarClient["credentials"][CalendarProvider.APPLE]
					: calendarClient["credentials"][providerId];
							...
							<div key={calendar.id} className="calendar-item">
								<label>
									<input
										...
										onChange={(e) => handleCalendarSelectionChange(provider, calendar.id, e.target.checked)}
									/>{" "}
		...
		return (
			<div className="sync-configuration">
				...
				<div className="destination-section">
					...
					{destination ? (
						<div className="destination-info">
							...
							<p>
								<strong>Provider:</strong> {destination.providerType}
							</p>
							<p>
								<strong>Calendar ID:</strong> {destination.calendarId}
							</p>
							<p>
								<strong>Conflict Resolution:</strong> {destination.conflictResolution}
							</p>
							<button onClick={() => setDestination(null)} disabled={loading} className="danger">
								Remove Destination
							</button>
						</div>
					) : (
								...
								onClick={() => {
									if (typeof document !== 'undefined') {
										...
										if (providerEl && calendarEl) {
											createDestinationFromSelection(providerEl.value, calendarEl.value);
										}
									}
								}}
								disabled={loading}>
						...
						<div className="sources-list">
							{syncSources.map((source) => (
								<div key={source.id} className="source-item">
									<h5>{source.name}</h5>
									<p>
										<strong>Provider:</strong> {source.providerType}
									</p>
									<p>
										<strong>Calendars:</strong> {source.calendars.length}
									</p>
									<p>
										<strong>Sync Direction:</strong> {source.syncDirection}
									</p>
									<p>
										<strong>Last Sync:</strong> {source.lastSync || "Never"}
									</p>
									<div className="source-actions">
										<button onClick={() => runSourceSync(source.id)} disabled={loading || !destination}>
											Sync Now
										</button>
										<button
											onClick={() => {
										...
										<button onClick={() => removeSyncSource(source.id)} disabled={loading} className="danger">
											Remove
										</button>
									</div>
								</div>
				...
				<div className="sync-actions">
					<button onClick={runSync} disabled={loading || !destination || syncSources.length === 0} className="primary">
			...
			<div className="events-list">
				...
				{events.map((event) => (
					<div key={event.id} className="event-item">
						<h4>{event.title}</h4>
						<p>
							<strong>When:</strong> {new Date(event.startTime).toLocaleString()} - {new Date(event.endTime).toLocaleString()}
						</p>
						<p>
							<strong>Calendar:</strong> {event.calendarName}
						</p>
						<p>
		...
		<div className="calendar-sync-component">
			...
			<div className="authentication-section">
				...
				<div className="auth-buttons">
					<button onClick={authenticateGoogle} disabled={loading || googleAuth} className={googleAuth ? "connected" : ""}>
						{googleAuth ? "Connected to Google" : "Connect Google Calendar"}
					</button>

					<button onClick={() => authenticateMicrosoft()} disabled={loading || microsoftAuth} className={microsoftAuth ? "connected" : ""}>
						{microsoftAuth ? "Connected to Microsoft" : "Connect Microsoft Calendar"}
					</button>

					<button onClick={authenticateApple} disabled={loading || appleAuth} className={appleAuth ? "connected" : ""}>
						{appleAuth ? "Connected to Apple" : "Connect Apple Calendar"}
					</button>

					<button onClick={authenticateExchange} disabled={loading || exchangeAuth} className={exchangeAuth ? "connected" : ""}>
						{exchangeAuth ? "Connected to Exchange" : "Connect Exchange Calendar"}
					</button>
				</div>

				{(googleAuth || microsoftAuth || appleAuth || exchangeAuth) && (
