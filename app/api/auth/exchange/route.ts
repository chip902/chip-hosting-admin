...
// Handle GET request to get Exchange auth URL
export async function GET(request: NextRequest) {
  const url = new URL(request.url);
  const searchParams = url.searchParams.toString();
  const targetUrl = `${CALENDAR_API_URL}/api/auth/exchange${searchParams ? `?${searchParams}` : ''}`;
  ...
  try {
    ...
    console.error('Error getting Exchange auth URL:', error);
    return NextResponse.json(
      { error: 'Failed to get Exchange authentication URL' },
      { status: 500 }
    );
  }
}