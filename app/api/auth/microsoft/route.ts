...
// Handle GET request to get Microsoft auth URL
export async function GET(request: NextRequest) {
  const url = new URL(request.url);
  const searchParams = url.searchParams.toString();
  const targetUrl = `${CALENDAR_API_URL}/api/auth/microsoft${searchParams ? `?${searchParams}` : ''}`;
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to get Microsoft authentication URL' },
      { status: 500 }
    );
  }
}