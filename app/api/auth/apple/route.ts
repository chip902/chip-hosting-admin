...
// Handle GET request to get Apple auth URL
export async function GET(request: NextRequest) {
  const url = new URL(request.url);
  const searchParams = url.searchParams.toString();
  const targetUrl = `${CALENDAR_API_URL}/api/auth/apple${searchParams ? `?${searchParams}` : ''}`;
  ...
  try {
    ...
    return NextResponse.json(
      { error: 'Failed to get Apple authentication URL' },
      { status: 500 }
    );
  }
}