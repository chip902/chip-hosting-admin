...
// Handle GET request for Apple OAuth callback
export async function GET(request: NextRequest) {
  const url = new URL(request.url);
  const searchParams = url.searchParams.toString();
  const targetUrl = `${CALENDAR_API_URL}/api/auth/apple/callback${searchParams ? `?${searchParams}` : ''}`;
  ...
  try {
    ...
    console.error('Error handling Apple OAuth callback:', error);
    return NextResponse.json(
      { error: 'Failed to process Apple authentication' },
      { status: 500 }
    );
  }
}