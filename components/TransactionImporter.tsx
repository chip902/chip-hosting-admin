...
const extractMetadata = (description: string, type: string): TransactionMetadata => {
...
const TransactionImporter: React.FC<TransactionImporterProps> = ({ userId, bankId }) => {
	...
	const handleFileUpload = (event: ChangeEvent<HTMLInputElement>) => {
		...
		Papa.parse<BankCSVRow>(selectedFile, {
			header: true,
			skipEmptyLines: true,
			...
			complete: (results: ParseResult<BankCSVRow>) => {
				...
				const validData: ImportedTransaction[] = results.data.map((row) => {
	...
	const renderMetadata = (metadata: TransactionMetadata) => {
		return (
						...
						([key, value]) =>
							value && (
								<div key={key} className="text-sm">
									<span className="font-medium text-gray-500 dark:text-gray-400">{key.replace(/([A-Z])/g, " $1").trim()}:</span>
				...
				<CardContent>
					<div className="flex flex-col items-center justify-center p-6 border-2 border-dashed rounded-lg">
						...
						<label htmlFor="file-upload" className="flex flex-col items-center cursor-pointer">
							...
							<span className="text-sm text-gray-600">{file ? file.name : "Drop CSV file here or click to upload"}</span>
						</label>
					</div>
													...
													{isExpanded && (
														<tr>
															<td colSpan={6} className="border-b">
																{renderMetadata(transaction.metadata)}
															</td>
														</tr>
													)}
												</React.Fragment>
					...
					{uploadStatus.status && (
						<Alert className={`mt-4 ${uploadStatus.status === "success" ? "bg-green-50" : uploadStatus.status === "error" ? "bg-red-50" : ""}`}>
